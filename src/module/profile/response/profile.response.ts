...
export abstract class SettingResponseData {
    @ApiProperty({
        default: '128fds',
    ...
    @ApiProperty({
        default: 150,
        description: '총 대화 시간(채팅 + 통화), 초 단위. guest을 제외한 host, speaker일 때 합계'
    })
    total_time: number;

    @ApiProperty({
        default: 150,
        description: '총 통화 시간(이제 필요없는듯), 초 단위. guest을 제외한 host, speaker일 때 합계'
    })
    total_call: number;

    @ApiProperty({
        default: 48,
        description: '대화 연결 횟수. guest을 제외한 host, speaker일 때 합계'
    })
    connection: number;
